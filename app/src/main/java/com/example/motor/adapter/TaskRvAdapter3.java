...
public class TaskRvAdapter3 extends RecyclerView.Adapter<TaskRvAdapter3.TaskViewHolder> {

    private static final String TAG = "zbb_TaskRvAdapter";
    private Context mContext;
    private List<TaskItemBean2> mTaskItemBeanList;
    private LayoutInflater mLayoutInflater;
    private boolean MultipleChoice = false;
    private String string2;

    public TaskRvAdapter3(Context context, List<TaskItemBean2> taskItemBeanList) {
        mContext = context;
    ...
    public void resetSelectedStatus() {
        for (TaskItemBean2 bean : mTaskItemBeanList) {
     ...
     * 获取当前选中的Item
     */
    public List<TaskItemBean2> getCurrentChoosenItems() {
        List<TaskItemBean2> temp = new ArrayList<>();
    ...
    public void onBindViewHolder(final TaskViewHolder holder, final int position) {
        ...
        final TaskItemBean2 bean = mTaskItemBeanList.get(position);
        if (null == bean) return;
        holder.tvTitle.setText(TextUtils.isEmpty(bean.getWOTitle()) ? "标题：无" : "标题：" + bean.getWOTitle());// 标题的赋值操作
        ...
        holder.tvWoid.setText(TextUtils.isEmpty(bean.getWOContent()) ? "内容：无" : "内容：" + bean.getWOContent());// 第三行描述的赋值操作
        holder.tvInfo.setText(TextUtils.isEmpty(bean.getWOExpectedTime()) ? "要求完成时间：无" :
                "要求完成时间：" + bean.getWOExpectedTime().replace("T", " "));// 第四行要求完成时间的赋值操作
        ...
        if(!TextUtils.isEmpty(bean.getWOState()) && bean.getWOState().equals("true")){
            holder.tvItemRightBtn.setVisibility(View.GONE);// 处理该任务按钮消失
                ...
                !bean.getWOIssuedUser().equals(userId)) {// 派发人不是当前用户
            holder.tvItemRightBtn.setVisibility(View.VISIBLE);// 处理该任务按钮显示
        } else if(!TextUtils.isEmpty(bean.getWOState()) && bean.getWOState().equals("false") &&
                bean.getWOIssuedUser().equals(userId)){ // 派发人是当前用户
            holder.tvItemRightBtn.setVisibility(View.GONE);// 处理该任务按钮消失
        }
        ...
        // 扩展开的3条摘要的赋值
        // 已处理
        if(bean.getWOType().equals("1") && bean.getWOState().equals("true")){
            if(TextUtils.isEmpty(bean.getWOReceiveUser())){
                holder.tvExtDesc.setText("接收人：没有转发，自己完成工单");
            } else {
                holder.tvExtDesc.setText("转发人：" + bean.getUserName());
            }
            holder.tvExtCompany.setText("完成情况：已完成");
            holder.tvExtPerson.setText(TextUtils.isEmpty(bean.getDeviceName()) ? "设备名称：无" : "设备名称：" + bean.getDeviceName());
        }
        // 已转发，派发人与当前用户一致
        if(bean.getWOType().equals("1") && bean.getWOState().equals("false") && bean.getWOIssuedUser().equals(userId)){
            // 已反馈是只要保证已反馈不为空就可以。已接收是要保证接收不为空并且反馈为空
            if(!TextUtils.isEmpty(bean.getWOFeedback())){
                // 已反馈
                holder.tvExtDesc.setText("转发情况：已转发");
                holder.tvExtCompany.setText(TextUtils.isEmpty(bean.getReceiveUser()) ? "反馈人：无" : "反馈人：" + bean.getReceiveUser());
