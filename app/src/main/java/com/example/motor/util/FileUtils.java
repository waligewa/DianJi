...
public class FileUtils {
    ...
    public static String SDPATH = MyApplication.SDPATH;
    ...
    //使用Bitmap加Matrix（矩阵）来缩放  如果图片的宽度小于degree不缩放  否则 按宽高比缩放
    ...
    public static int Counting(String text, String sub){
        int count =0, start =0;
    ...
    public static Bitmap compressImage(Bitmap image) {
        ...
        Log.e("===图片大小yuan===", baos.toByteArray().length / 1024 + "K");
        while (baos.toByteArray().length / 1024 > 1536) { // 循环判断如果压缩后图片是否大于2M,大于继续压缩
            baos.reset(); // 重置baos即清空baos
            image.compress(Bitmap.CompressFormat.JPEG, options, baos);// 这里压缩options%，把压缩后的数据存放到baos中
            options -= 2;// 每次都减少10
            Log.e("===图片大小"+options+"===", baos.toByteArray().length / 1024 + "K");
        }
    ...
    public static Bitmap compressScale(Bitmap image) {
        ...
        if (baos.toByteArray().length / 1024 > 1024*1.5) {
            baos.reset();// 重置baos即清空baos
