...
public class TagAliasOperatorHelper {
            ...
            switch (msg.what){
                ...
                case DELAY_SET_MOBILE_NUMBER_ACTION:
                    if(msg.obj != null && msg.obj instanceof String) {
                        Log.e(TAG, "retry set mobile number");
                        sequence++;
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        Log.e(TAG,"action - onTagOperatorResult, sequence:" + sequence + ",tags:" +
                jPushMessage.getTags());
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        Log.e(TAG,"action - onCheckTagOperatorResult, sequence:" + sequence + ",checktag:"
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        Log.e(TAG,"action - onAliasOperatorResult, sequence:" + sequence + ",alias:"
        ...
        if(jPushMessage.getErrorCode() == 0){
            ...
            String logs = "Failed to " + getActionStr(tagAliasBean.action)
                    + " alias, errorCode:" + jPushMessage.getErrorCode();
            Log.e(TAG, logs);
            if(!RetryActionIfNeeded(jPushMessage.getErrorCode(), tagAliasBean)) {
                // 会弹出toast：Failed to set alias, errorCode:6002，感觉不利于用户体验，
                // 因此就只看控制台吧，不弹出toast了
                //ExampleUtil.showToast(logs, context);
            }
        }
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        Log.e(TAG,"action - onMobileNumberOperatorResult, sequence:" + sequence + ",mobileNumber:"
                + jPushMessage.getMobileNumber());
        init(context);
        if(jPushMessage.getErrorCode() == 0){
            Log.e(TAG,"action - set mobile number Success,sequence:" + sequence);
            setActionCache.remove(sequence);
        }else{
