...
import com.example.motor.adapter.WheelAdapter;
import com.example.motor.listener.OnWheelChangedListener;
import com.example.motor.listener.OnWheelScrollListener;
...
public class WheelView extends View {
	...
	/** Minimum delta for scrolling 用于滚动的最小增量*/
	private static final int MIN_DELTA_FOR_SCROLLING = 1;

	/** Current value & label text color 当前值和标签文本颜色*/
	private static final int VALUE_TEXT_COLOR = 0xff000000;

	/** Items text color 项目的文本颜色*/
	private static final int ITEMS_TEXT_COLOR = 0x66000000;

	/** Top and bottom shadows colors 顶部和底部阴影颜色，这是数字在的位置*/
	private static final int[] SHADOWS_COLORS = new int[] { 0xffff, 0xffff, 0xffff };

    /*private static final int[] SHADOWS_COLORS = new int[] { 0xFF111111,
            0x00AAAAAA, 0x00AAAAAA };//这个是矩形的灰白相间的那个框框*/
	...
	/** Top and bottom items offset (to hide that) 顶部和底部的项目偏移（隐藏）*/
	private final int ITEM_OFFSET = TEXT_SIZE / 5;

	/** Additional width for items layout 项目布局的附加宽度*/
	private static final int ADDITIONAL_ITEMS_SPACE = 10;

	/** Label offset 标签偏移*/
	private static final int LABEL_OFFSET = 8;

	/** Left and right padding value 左右填充值*/
	private static final int PADDING = 10;

	/** Default count of visible items 可见项的默认计数*/
	...
	private void initData(Context context) {
		gestureDetector = new GestureDetector(context, gestureListener);     //手势探测器
	...
	public void setAdapter(WheelAdapter adapter) {
		...
		invalidateLayouts();          //invalidate使无效
		invalidate();
	}
	 ...
	 *            the interpolator
	 */
	public void setInterpolator(Interpolator interpolator) {       // 插补器
	...
	private SimpleOnGestureListener gestureListener = new SimpleOnGestureListener() {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
			...
			scroller.fling(0, lastScrollY, 0, (int) -velocityY / 2,
                    0, 0, minY, maxY);
